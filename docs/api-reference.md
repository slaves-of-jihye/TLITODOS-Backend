...
루틴 생성/전환/규칙 조회/삭제는 소유자만 가능합니다. requestId 재시도는 같은 본문으로 보내며, 바뀐 본문은 409, 삭제된 루틴 재시도는 410입니다. 삭제는 전체 회차 삭제입니다. 일괄 생성은 한 트랜잭션이며 최대 1,000회차. 자세한 반복 규칙과 JS 예제는 문서 후반에 포함했습니다.
...
루틴에 속한 Todo이면 해당 루틴의 모든 회차를 삭제하고, 일반 Todo 응답 대신 `{"success":true,"routineId":12,"deletedCount":3}` 형태를 반환합니다.
...
- 루틴 삭제는 완료 여부와 무관하게 모든 회차를 삭제합니다.
...
- `DELETE /api/v1/todos/routines/{routineId}`는 루틴의 **완료된 것까지 모든 Todo**를 삭제합니다.
  루틴에 속한 Todo의 기존 DELETE도 전체 삭제로 연결됩니다. 프론트는 반드시 전체 삭제 안내를 보여주세요.
- 개별 Todo PATCH는 해당 회차에만 적용됩니다. 전체 반복 규칙 수정 UI가 없어 전체 규칙 PATCH는 추가하지 않았습니다.
- 삭제 후 늦게 도착한 생성 재시도로 복원되지 않도록 내부 요청 기록은 tombstone으로 보존합니다.
